...
async function buyNFT(amount) {
    ...
    contractMonstro.methods.FreeMint(amount).send(
        {
            from: currentAccount,
            to: CONTRACT_ADDRESS_MONSTRO,
            value: await contractMonstro.methods.getFreeMintPrice().call() * amount
        })
        ...
        .then(
            res => {
                console.log(res);
                resetCheckout();
                showToast(`Transaction confirmed in block #${res.blockNumber} - Waaa! CHECK YOUR BABIES!`, "success", 6000);
                showSuccessDialog();
            })
        .catch(
            err => {
...
async function showSuccessDialog() {
    if (currentAccount != null) {
        if (currentPage === "wallet") {
            ...
            $('#your_monstro').text("Your Winter Babies: " + MonstroCount + "");
...
async function getNFTstatus() {
    if (contractsInitialized) {
            ...
            monstroPrice = web3.utils.fromWei(await contractMonstro.methods.getFreeMintPrice().call(), 'ether');
            ...
            $('#your_monstro').text("Your Winter Babies: " + monstroCount + "");
            monstroWL = await contractMonstro.methods.isinWhielist(currentAccount).call();
            $('#your_wl').text("Your WhiteList Status: " + monstroWL + "");
...
async function loadWallet() {
    ...
    $('#your_monstro').text("Your Winter Baby Apes: " + monstroCount + "");
